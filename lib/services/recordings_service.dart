...
class RecordingsService {
      ...
      if (response.statusCode == 200) {
        try {
          ...
          await LogService.log(
            ...
            details:
                'Received ${jsonData.length} recordings, response size: ${response.body.length} bytes',
            metadata: {
          ...
          await LogService.log(
            ...
            details:
                'Found ${recordings.length} recordings for ${date.toLocal().toString().split(' ')[0]}',
            metadata: {
        ...
        await LogService.log(
          ...
          details:
              'No recordings available for ${date.toLocal().toString().split(' ')[0]}',
        );
        return [];
      } else {
        await LogService.log(
  ...
  static Future<String> downloadCustomSegment({
    ...
    await LogService.log(
      ...
      details:
          'Starting download: ${durationSeconds.round()}s segment at ${offsetSeconds.round()}s offset',
      metadata: {
    ...
    try {
      ...
      final formattedTime = startTime
          ...
          .substring(0, 19);
      final fileName =
          'dashcam_${formattedTime}_${durationSeconds.round()}s.mp4';
      ...
      await LogService.log(
        ...
        details:
            'Successfully downloaded ${(response.bodyBytes.length / 1024 / 1024).toStringAsFixed(2)} MB',
        metadata: {
          'fileName': fileName,
